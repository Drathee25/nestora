...
export default function PropertyDetail() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-16">
      <div className="rounded-xl overflow-hidden bg-[#1f4d36]/5 aspect-video mb-3">
        {images[activeImage] ? (
          <img src={images[activeImage]} alt={property.title} className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-[#1f4d36]/30">No image</div>
        )}
      </div>
      {images.length > 1 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        <div className="md:col-span-2">
          ...
          <h1 className="font-display text-3xl text-[#1f4d36] mt-1">{property.title}</h1>
          <p className="text-[#1f4d36]/60 mt-1">{property.address}</p>

          <div className="flex gap-6 mt-6 text-sm text-[#1f4d36]/70">
            <span>{property.bedrooms} bedrooms</span>
            <span>{property.bathrooms} bathrooms</span>
            <span>{property.areaSqft} sqft</span>
          </div>

          <p className="mt-6 leading-relaxed text-[#1f4d36]/80 whitespace-pre-line">
        ...
        <aside className="border border-[#1f4d36]/10 rounded-xl p-6 h-fit bg-white">
          <h3 className="font-medium mb-2 text-[#1f4d36]">Interested in this property?</h3>
          <p className="text-sm text-[#1f4d36]/60 mb-4">
            Call or message us directly to get more details or schedule a viewing.
          </p>
          <div className="space-y-3">
            <a
              href={`tel:${CONTACT_PHONE_TEL}`}
              className="flex items-center justify-center gap-2 px-6 py-3 bg-[#1f4d36] text-[#faf3e7] rounded-full text-sm hover:bg-[#143527] transition-colors"
            >
